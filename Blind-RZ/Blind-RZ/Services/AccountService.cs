...
namespace Blind_RZ.Services
{
    ...
    {
        //private readonly BlindsContext _context;
        ...
        {
            var account = _accountRepository.SingleOrDefault(x => x.Email == model.Email);
        ...
        {
            // validate
            if (_accountRepository.Any(x => x.Email == model.Email))
            {
        ...
        {
            var account = _accountRepository.SingleOrDefault(x => x.VerificationToken == token);
        ...
        public Account GetAccountByEmail(AuthenticateRequest request)
        {
            var account = _accountRepository.GetAll().Include(account => account.Commands).FirstOrDefault(account => account.Email == request.Email);
            return account;
        }
        ...
        public AccountResponse Update(Guid id, UpdateRequest model)
        {
            ...
            // validate
            if (account.Email != model.Email && _accountRepository.Any(x => x.Email == model.Email))
                throw new AppException($"Email '{model.Email}' is already registered");
        ...
        {
            var account = _accountRepository.SingleOrDefault(u => u.RefreshTokens.Any(t => t.Token == token));
        ...
        {
            var account = _accountRepository.SingleOrDefault(x =>
        ...
        private string generateResetToken()
        {
            ...
            var tokenIsUnique = !_accountRepository.GetAll().Any(x => x.ResetToken == token);
            if (!tokenIsUnique)
        ...
        private string generateVerificationToken()
        {
            ...
            var tokenIsUnique = !_accountRepository.GetAll().Any(x => x.VerificationToken == token);
            if (!tokenIsUnique)
